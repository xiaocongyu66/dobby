...
#include "elf/dobby_xdl_symbol_resolver.h"
#endif
...
PUBLIC void *DobbySymbolResolverEx(const char *image_name, const char *symbol_name_pattern, uint32_t flags,
                                  size_t *symbol_size) {
  if (symbol_size)
    *symbol_size = 0;
  if (!symbol_name_pattern || symbol_name_pattern[0] == '\0')
    return NULL;

  void *result = NULL;

#if defined(__ANDROID__) && defined(DOBBY_ANDROID_USE_XDL)
  result = dobby_xdl_resolve_symbol_ex(image_name, symbol_name_pattern, flags, symbol_size);
  if (result)
    return result;

  // If the caller explicitly selected a specific xDL table, do not silently
  // fall through to dlsym or mmap-based ELF scanning.
  if (flags & (DOBBY_SYMBOL_RESOLVER_DYNSYM_ONLY | DOBBY_SYMBOL_RESOLVER_SYMTAB_ONLY))
    return NULL;
#endif
  ...
  if (!image_name || image_name[0] == '\0') {
    ...
    if (handle) {
      result = dlsym(handle, symbol_name_pattern);
      dlclose(handle);
      if (result)
        return result;
    }
...
PUBLIC void *DobbySymbolResolver(const char *image_name, const char *symbol_name_pattern) {
  return DobbySymbolResolverEx(image_name, symbol_name_pattern, 0, NULL);
}

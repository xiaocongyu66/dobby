...
struct WaitTask {
    ...
    dobby_wait_callback_t cb = nullptr;
...
static bool try_install(WaitTask &t) {
    void *h = xdl_open(t.lib.c_str(), XDL_DEFAULT);
    if (!h)
        return false;
    void *sym = nullptr;
    if (!t.use_offset) {
        sym = xdl_sym(h, t.symbol.c_str(), nullptr);
        if (!sym)
            sym = xdl_dsym(h, t.symbol.c_str(), nullptr);   // symtab/gnu_debugdata
    } else {
        // 库基址 + 偏移
        DobbyAndroidHookRecord rec;
        (void)rec;
        // xDL 无直接 base API — 用 dl_iterate 简化: GlossOpen 系也基于 xdl,
        // 这里直接用 DobbyAndroidGetModuleBase (dobby_android_api 已实现)
        sym = (void *)DobbyAndroidGetModuleBase(t.lib.c_str());
        if (sym)
            sym = (void *)((uintptr_t)sym + t.offset);
    }
    xdl_close(h);
    if (!sym)
        return false;
    // 装配!
    DobbyHook(sym, t.replace, t.origin);
    t.done = true;
    if (t.cb)
        t.cb(DOBBY_WAIT_STATUS_INSTALLED, t.lib.c_str(), t.symbol.c_str(), sym, t.user_data);
    return true;
}
    ...
    for (;;) {
        ...
        for (auto it = g_tasks.begin(); it != g_tasks.end();) {
            ...
            if (it->timeout_ms && now - it->start_ms > it->timeout_ms) {
                if (it->cb)
                    it->cb(DOBBY_WAIT_STATUS_TIMEOUT, it->lib.c_str(), it->symbol.c_str(), nullptr, it->user_data);
                it = g_tasks.erase(it);
                continue;
            }
            try_install(*it);
...
int Waiter::Submit(const char *lib, const char *symbol, uintptr_t offset, bool use_offset,
    ...
    if (!lib || !replace)
        return DOBBY_WAIT_STATUS_INVALID;
    ensure_worker();
    std::lock_guard<std::mutex> lk(g_tasks_mutex);
    WaitTask t;
    t.lib = lib;
    ...
    if (try_install(t))          // 已加载 → 立即装配
        return DOBBY_WAIT_STATUS_INSTALLED;
    g_tasks.push_back(t);
    return DOBBY_WAIT_STATUS_PENDING;
}
...
int Waiter::Cancel(const char *lib, const char *symbol) {
    ...
    for (auto it = g_tasks.begin(); it != g_tasks.end();) {
        ...
        if (match) {
            if (it->cb)
                it->cb(DOBBY_WAIT_STATUS_CANCELLED, it->lib.c_str(), it->symbol.c_str(), nullptr, it->user_data);
            it = g_tasks.erase(it);
            n++;
        } else
            ++it;
